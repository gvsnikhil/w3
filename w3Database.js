...
function alivaWebSQLInit() {
    var alivaDBObj = this;
    alivaDatabase = window.openDatabase('alivaSchoolsDemoDatabase', '1.0', 'alivaSchoolsDemoDatabase', 2 * 1024 * 1024);
    function alivaDropTable(tablename) {
        var sql="DROP TABLE [" + tablename + "]";
        alivaDatabase.transaction(function (tx)
            {
                tx.executeSql(sql,[]);
            }
            ,function (err) {
    ...
    function alivaDropView(tablename) {
    ...
    function alivaDropIndex(tablename) {
    ...
    this.alivaExecuteSQL = function(sql) {
        var resultContainer;
    ...
    this.selectStar = function (tablename) {
        ...
        alivaDBObj.alivaExecuteSQL(sql);
    };
        ...
        alivaDatabase.transaction(function (tx)
            {
                ...
                function alivaDBInfo() {
                    ...
                    for (i = 0; i < tblnames.length; i++) {
                        txt = txt + "<tr>";       
                        txt = txt + "<td title='Click to see the content of the " + tblnames[i] + " table' style='text-align:left;cursor:pointer;text-decoration:underline;' onclick='alivaschoolsWebSQL1.selectStar(\"" + tblnames[i] + "\")'>" + tblnames[i] + "</td>";  
                        txt = txt + "<td style='text-align:right;'>" + recordcounts[i] + "</td>";                  
                        txt = txt + "</tr>";       
                    }
                    document.getElementById("yourDB").innerHTML =  txt + "</table>";
                }
                function alivaDBViewInfo() {
                    var txt = "", i;
                    txt = txt + "<h4>Views:</h4>";
                    txt = txt + "<table width='100%' xclass='ws-table-all notranslate'><tr>";
                    ...
                    for (i = 0; i < viewnames.length; i++) {
                        txt = txt + "<tr>";       
                        txt = txt + "<td title='Click to see the content of the " + viewnames[i] + " view' style='text-align:left;cursor:pointer;text-decoration:underline;' onclick='alivaschoolsWebSQL1.selectStar(\"" + viewnames[i] + "\")'>" + viewnames[i] + "</td>";  
                        txt = txt + "<td style='text-align:right;'>" + viewrecordcounts[i] + "</td>";                  
                        txt = txt + "</tr>";       
                    }
                    document.getElementById("yourRC").innerHTML =  txt + "</table>";
                }
                function alivaDBIndexInfo() {
                    var txt = "", i;
    ...
    this.alivaInitDatabase = function(n) {
        alivaDBObj.alivaInitCustomers();
        alivaDBObj.alivaInitCategories();
        alivaDBObj.alivaInitEmployees();
        alivaDBObj.alivaInitOrderDetails();    
        alivaDBObj.alivaInitOrders();        
        alivaDBObj.alivaInitProducts();            
        alivaDBObj.alivaInitShippers();                
        alivaDBObj.alivaInitSuppliers(n);    
    };
    this.alivaClearDatabase = function() {
                    ...
                    tx.executeSql("SELECT name FROM sqlite_master WHERE type='index' AND name<>'sqlite_autoindex___WebKitDatabaseInfoTable___1'",[],function (tx, results)
                        {
                            ...
                            if (len>0) {
                                for (i = 0; i < len; i++) {
                                    alivaDropIndex(results.rows.item(i).name);
                                }
                            }
                        }
                    );
                    tx.executeSql("SELECT name FROM sqlite_master WHERE type='view'",[],function (tx, results)
                        {
                            ...
                            if (len>0) {
                                for (i = 0; i < len; i++) {
                                    alivaDropView(results.rows.item(i).name);
                                }
                            }
                        }
                    );
                    tx.executeSql("SELECT name FROM sqlite_master WHERE type='table' AND name<>'sqlite_sequence' AND name<>'__WebKitDatabaseInfoTable__'",[],function (tx, results)
                        {
                            ...
                            if (len>0) {
                                for (i = 0; i < len; i++) {
                                    alivaDropTable(results.rows.item(i).name);
                                    if (i === (len - 1)) { alivaDBObj.alivaInitDatabase(1); }
                                }
                            } else {
                                alivaDBObj.alivaInitDatabase(1);
                            }
                        }
                    );
                }
    ...
    this.alivaInitCategories = function() {
        alivaDatabase.transaction(function (tx)
            {
    ...
    this.alivaInitCustomers = function() {
        alivaDatabase.transaction(function (tx)
            {
    ...
    this.alivaInitEmployees = function() {
        alivaDatabase.transaction(function (tx)
            {
    ...
    this.alivaInitOrderDetails = function() {
        alivaDatabase.transaction(function (tx)
            {
    ...
    this.alivaInitProducts = function() {
        alivaDatabase.transaction(function (tx)
            {
    ...
    this.alivaInitShippers = function() {
        alivaDatabase.transaction(function (tx)
            {
    ...
    this.alivaInitSuppliers = function(n) {
        alivaDatabase.transaction(function (tx)
            {
    ...
    this.runSQL = function(n) {
        alivaDatabase.transaction(function (tx)
            ...
            ,function (err) {
                alivaDBObj.alivaInitDatabase(0);
            }
        );
    };
